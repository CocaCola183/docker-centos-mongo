...
# 启动并配置分片1
nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/mongod-1-1/mongo.conf &
waitmongo $SHARD1_MASTER_ADDRESS

nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/mongod-1-2/mongo.conf &
waitmongo $SHARD1_SECONDARY_ADDRESS

nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/mongod-1-3/mongo.conf &
waitmongo $SHARD1_ARBIT_ADDRESS

/opt/mongo-source/mongodb-2.4.3/bin/mongo \
...
# /opt/mongo-source/mongodb-2.4.3/bin/mongo 127.0.0.1:7001/admin

# 启动并配置分片2
nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/mongod-2-1/mongo.conf &
waitmongo $SHARD2_MASTER_ADDRESS

nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/mongod-2-2/mongo.conf &
waitmongo $SHARD2_SECONDARY_ADDRESS

nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/mongod-2-3/mongo.conf &
...
# /opt/mongo-source/mongodb-2.4.3/bin/mongo 127.0.0.1:7003/admin

# 启动并配置分片3
nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/mongod-3-1/mongo.conf &
waitmongo $SHARD3_MASTER_ADDRESS

nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/mongod-3-2/mongo.conf &
waitmongo $SHARD3_SECONDARY_ADDRESS

nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/mongod-3-3/mongo.conf &
...
# /opt/mongo-source/mongodb-2.4.3/bin/mongo 127.0.0.1:7005/admin

# 启动配置节点
nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/config-1/mongo.conf &
waitmongo $CONFIG1_ADDRESS
# tail -f /mongodb/config-1/log/mongo.log
 
nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/config-2/mongo.conf &
waitmongo $CONFIG2_ADDRESS
# tail -f /mongodb/config-2/log/mongo.log

nohup /opt/mongo-source/mongodb-2.4.3/bin/mongod -f /data/config-3/mongo.conf &
...
# # 启动并配置mongos_1
nohup /opt/mongo-source/mongodb-2.4.3/bin/mongos -f /data/mongos-1/mongo.conf &
...
# /opt/mongo-source/mongodb-2.4.3/bin/mongo 127.0.0.1:9000

# # 启动并配置mongos_2
nohup /opt/mongo-source/mongodb-2.4.3/bin/mongos -f /data/mongos-2/mongo.conf &
